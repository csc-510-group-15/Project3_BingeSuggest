...
async function renderWishlist() {
    ...
    wishlist.forEach(entry => renderWishlistEntry(entry, wishlistContainer));
}

async function renderWishlistEntry(entry, wishlistContainer){
    ...
    var movieData;
    try{
        movieData = await fetchMovieData(entry.imdb_id);
    } catch(error){
        console.error(error);
    }
    ...
    if((entry.score * 10) % 10 > 0){
        reviewDiv.append(halfStar);
    }

    var commentDiv = $('<div>').addClass('comment').text(entry.review);
    ...
    wishlistContainer.append(postDiv);
}
