...
def test_boot_to_login_page_status(client):
    """Test wishlist page boots you to login if you're not signed on and returns 200 status code"""
    srcapp.user[1] = None
    response = client.get("/wishlist")
    assert response.status_code == 200
...
def test_wishlist_page_content(client):
    ...
    assert srcapp.user[1] is not None
    assert "My Wishlist" in data
    assert "Search for a Movie" in data
    assert "Add" in data


def test_boot_to_login_page_content(client):
    """Test wishlist page boots you to login if you're not signed on and contains correct elements"""
    srcapp.user[1] = None
    response = client.get("/wishlist")
    data = response.get_data(as_text=True)
    # Data here is currently the login page because user[1] == None
    assert srcapp.user[1] is None
...
def test_wishlist_retrive_name(client):
...
def test_wishlist_retrieve_id(client):
    """Test wishlist search functionality"""
    srcapp.user[1] = 1
    response = client.get("/getWishlistData")
    data = response.get_data(as_text=True)
    assert "tt1234567" in data


# def test_wishlist_item_count(client):
#     """Test wishlist item count is accurate"""
#     # Add 3 items
#     client.post("/add_to_wishlist", json={"movieName": "Jumanji (1995)"})
